...
#Initializes the Discord Bot to begin functionality in Python
...
#Program prints line to command terminal to confirm Discord Bot functionality
@bot.event
async def on_ready():
    print(f'Client has logged on as {bot.user}')

#Test feature for ping, which is not accessible to the user
@bot.command()
async def ping(ctx):
